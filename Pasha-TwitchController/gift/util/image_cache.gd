...
func _init(a_do_disk_cache : bool, a_cache_path : String) -> void:
	self.disk_cache = a_do_disk_cache
	self.cache_path = a_cache_path
	var _unused = thread.start(self, "start")

func start(_params) -> void:
	var f : File = File.new()
	...
	if (disk_cache):
		for type in caches.keys():
			...
			caches[cache_dir] = {}
			var _error := d.make_dir_recursive(cache_path + "/" + cache_dir)
	while active:
		if (!queue.empty()):
			mutex.lock()
			...
			if (disk_cache):
				if !d.dir_exists(entry.filename.get_base_dir()):
					var _unused = d.make_dir(entry.filename.get_base_dir())
				var _unused = f.open(entry.filename, File.WRITE)
				f.store_buffer(buffer)
				f.close()
			var _texture = ImageTexture.new()
			var img : Image = Image.new()
			var _unused = img.load_png_from_buffer(buffer)
...
func get_badge_mapping(channel_id : String = "_global") -> Dictionary:
	if !caches[RequestType.BADGE_MAPPING].has(channel_id):
		...
		if !disk_cache && file.file_exists(filename):
			var _unused = file.open(filename, File.READ)
		...
		if !buffer.empty():
			...
			if (disk_cache):
				var _unused = file.open(filename, File.WRITE)
...
func get_badge(badge_name : String, channel_id : String = "_global", scale : String = "1") -> ImageTexture:
	...
	if !caches[RequestType.BADGE][channel_id].has(cachename):
		if !disk_cache && file.file_exists(filename):
			var _unused = file.open(filename, File.READ)
			var img : Image = Image.new()
			_unused = img.load_png_from_buffer(file.get_buffer(file.get_len()))
			...
			if !map.empty():
				if map.has(badge_data[0]):
					...
					var img = preload("res://mods-unpacked/Pasha-TwitchController/gift/placeholder.png")
					texture.create_from_image(img)
...
func http_request(path : String, type : int) -> PoolByteArray:
	...
	if (host != new_host):
		...
		if (error != OK):
			...
			return buffer
		host = new_host
	var _unused = http_client.request(HTTPClient.METHOD_GET, path, HEADERS)
	while (http_client.get_status() == HTTPClient.STATUS_REQUESTING):
		_unused = http_client.poll()
		delay(50)
...
class Entry extends Reference:
	...
	func _init(a_path : String, a_type : int, a_filename : String, a_data : Array):
		self.path = a_path
		self.type = a_type
