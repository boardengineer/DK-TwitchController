...
const ImageCache  = preload("res://mods-unpacked/Pasha-TwitchController/gift/util/image_cache.gd")
const CommandData  = preload("res://mods-unpacked/Pasha-TwitchController/gift/util/cmd_data.gd")
const CommandInfo  = preload("res://mods-unpacked/Pasha-TwitchController/gift/util/cmd_info.gd")
...
func remove_command(cmd_name : String) -> void:
	var _unused = commands.erase(cmd_name)
...
func purge_command(cmd_name : String) -> void:
	...
	if(to_remove):
		...
		for queued in remove_queue:
			var _unused = commands.erase(queued)
...
func connection_closed(was_clean_close : bool) -> void:
	print_debug("clean close? ", was_clean_close)
